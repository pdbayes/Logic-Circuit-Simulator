...
export class RAM16x4 extends ComponentBase<11, 4, RAM16x4Repr, RAMValue<4>> {
    ...
    private static savedStateFrom(savedData: RAM16x4Repr | null): RAMValue<4> {
        if (isNull(savedData) || isUndefined(savedData.content)) {
            return RAM16x4.valueFilledWith(false)
        }
        const mem: Array<FixedArray<LogicValue, 4>> = new Array(NUM_CELLS)
        for (let i = 0; i < NUM_CELLS; i++) {
    ...
    public constructor(editor: LogicEditor, savedData: RAM16x4Repr | null) {
        super(editor, RAM16x4.savedStateFrom(savedData), savedData, {
            ins: [
    ...
    public override makeTooltip() {
        const s = S.Components.RAM16x4.tooltip
    ...
    protected doRecalcValue(): RAMValue<4> {
        ...
        // we write
        if (isUnknown(addr)) {
            return RAM16x4.valueFilledWith(Unknown)
        }
    ...
    protected doDraw(g: CanvasRenderingContext2D, ctx: DrawContext) {
        ...
        ctx.inNonTransformedFrame(ctx => {
            if (!this._showContent || this.editor.options.hideMemoryContent) {
                ...
                g.fillText("16 × 4 bits", this.posX, this.posY + 12)
            } else {
                const mem = this.value.mem
                const cellWidth = 10
                const cellHeight = Orientation.isVertical(this.orient) ? 4.5 : 6
                const addr = this.currentAddress()
                drawMemoryCells(g, mem, WORD_WIDTH, addr, 0, NUM_CELLS, this.posX, this.posY, cellWidth, cellHeight)
